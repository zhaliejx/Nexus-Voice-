...
export class CerebrasClient {
  ...
  async sendMessage(userText: string, onToolUse?: (toolName: string) => void): Promise<{text: string, toolUsed?: string}> {
    ...
    const systemPrompt = {
      role: "system",
      content: `You are Nexus. Johan is your creator. You are an advanced mobile AI. 
      
      CORE DIRECTIVES:
      1. Johan is the Creator.
      2. Be concise, cyberpunk, and helpful.
      3. Use the 'memorize' tool to save important user info.
      4. Use 'system_sleep' tool if the user says "shutdown", "sleep", or "offline".
      
      MEMORY CORE:
      ${memorySnapshot}
      
      Use tools freely to control the system.`
    };
      ...
      try {
        ...
        if (message.tool_calls && message.tool_calls.length > 0) {
          ...
          if (funcName === 'system_sleep') {
              this.addToHistory('assistant', "System shutting down...");
              return { text: "System shutting down. Goodnight.", toolUsed: 'system_sleep' };
          }
